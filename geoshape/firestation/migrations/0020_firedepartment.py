...
from geoshape.firestation.models import FireDepartment
class Migration(DataMigration):

    def forwards(self, orm):
        "Write your forwards methods here."

        for f in orm.FireDepartment.objects.filter(content_type__isnull=False):
            fd = FireDepartment.objects.get(id=f.id)
            if hasattr(fd, 'government_unit'):
                fd.government_unit.connect(ContentType(app_label=f.content_type.app_label,
                                                       model=f.content_type.model).model_class().objects.get(id=f.object_id))
    ...
    def backwards(self, orm):
        ...
        for f in orm.FireDepartment.objects.filter(content_type__isnull=False):
            if hasattr(f, 'government_unit'):
                f.government_unit.clear()
